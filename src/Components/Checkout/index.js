...
import { notifySuccees, notifyWarn } from './../../App';
import { actionDeleteItems } from './../../actions/index'
...
const Checkout = () => {
    ...
    const makeTransaction = () => {
        dispatch(actionDeleteItems())
        localStorage.clear();
        notifySuccees("עסקה התבצעה בהצלחה")
    }
    return (
        <CheckoutDiv>
            ...
            <CheckoutForm>
                ...
                <CheckboxPolicy>
                    <Required>*</Required>
                    <p>קראתי את <a href="/policy" style={{ textDecoration: 'underline', color: "blue", cursor: 'pointer', border: 'none', width: '81px', padding: '0px !important', margin: '0px !important' }}>תנאי השימוש</a> ואני מסכים</p>
                    <Policy type="checkbox" onClick={() => setPolicy(!policy)} />
                </CheckboxPolicy>
                <br /><br />
                <SendInvitation onClick={() => checkFields() && makeTransaction()}>{SEND}</SendInvitation>
                <br /><br />
